...

            {
                type === 'service' && 
                    filteredAndSortedList.map((item: any, index:number) => (
                        <div className="w-full flex items-center justify-start gap-2 bg-background px-1 py-4 rounded-lg cursor-pointer " onClick={() => setSelect(item)} key={item.name+index}>
                            <Stars
