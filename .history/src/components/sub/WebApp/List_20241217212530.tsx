...
export const List = ({ filter, favorites, setFavorites, setSelect, type, service, filterSelect}: ListProps) => {
    ...
    const filteredAndSortedList = useMemo(() => {
        ...
        if (type === 'service') {
            ...
            const uniqueServices = serviços.filter((item: any, index: number, self: any[]) => {
            ...
            return uniqueServices
                .filter((item: any) => {
                    ...
                    const hasFilterSelectId = filterSelect?.id !== undefined; // Verifica se filterSelect possui um id
                    const matchesFilter = item.name.toLowerCase().includes(filter.toLowerCase());
                
                    // Se filterSelect.id existir, adiciona a verificação, senão só considera o filtro pelo nome
                    return matchesFilter && (!hasFilterSelectId || item.country_id === filterSelect.id);
                })
        ...
        if (type === 'country') {
            ...
            return list
                .filter((item: any) => {
                    const hasFilterSelectId = filterSelect?.id !== undefined; // Verifica se filterSelect possui um id
                    const matchesFilter = item.country_name.toLowerCase().includes(filter.toLowerCase());
    ...
    }, [list, type, serviços, filterSelect.id, filter, favorites]);
