...
import { ListOperadoras } from "./ListOperadoras";
    ...
    if (type === 'operator' && operator) {
      return (
        <div className="flex flex-col gap-2 relative  ">
          <span className="text-xl w-full text-start px-5 text-black font-ubuntu ">{subTitle}</span>
          {
            operatorItems ?
            <div className="max-h-96 shadow-inner relative bg-background shadow-black py-2 w-full my-2 overflow-x-auto scroll-smooth rounded-lg px-2 text-black font-ubuntu flex flex-col gap-2 ">
              <Suspense fallback={<SkeletonList />}>
                <ListOperadoras type={type} filter={filter} favorites={favorites} setFavorites={setFavorites} setSelect={setSelect} operatorItems={operatorItems} filterSelect={filterSelect} />
              </Suspense>
              {/* {
                type === 'operator' &&
    ...
    return (
      <div className="flex flex-col gap-2 ">
        ...
        { 
          ...
          :
          <>
            <Input type="text" placeholder="Pesquisar" size="lg" classNameInput="w-full h-12 " register={register("filter")} />
            <div className="max-h-96 shadow-inner relative bg-background shadow-black py-2 w-full my-2 overflow-x-auto scroll-smooth rounded-lg px-2 text-black font-ubuntu flex flex-col gap-2 ">
              <Suspense fallback={<SkeletonList />}>
