...
export const List = ({ filter, favorites, setFavorites, setSelect, type, service}: ListProps) => {
    ...
    const serviços = list.map((item: any) =>  {
        return {
            ...
            short: item.services.short,
            logo: item.services.logo
        }
    }).flat()
    ...
    const filteredAndSortedList = useMemo(() => {
        if (!list) return [];
        if(type === 'service') {
            // {cost: 8, count: 22886, logo: '/images/services/baa.webp', name: 'Wirex', short: 'baa'}
            return list.filter((item: any) => item.title.toLowerCase().includes(filter.toLowerCase()))
