...

            {
                type === 'service' && 
                    filteredAndSortedList.map((item: any, index:number) => (
                        <div className="w-full flex items-center justify-start gap-2  " key={item.name+index}>
                            <Stars
                                stars={favorites.includes(item.name)}
                                setFavorite={() => setFavoriteInStorage(item.name)}
                            />
                            <Image src={`${baseUrl}${item.logo}`} alt={item.name} width={30} height={30} className="rounded-full shadow-sm shadow-black min-w-8 h-8 " />
                            <button 
