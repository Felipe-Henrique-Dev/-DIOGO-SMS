...
export const List = ({ filter, favorites, setFavorites, setSelect, type, service}: ListProps) => {
    ...
    }, [list, type, serviços, filter, favorites]);
...

            {
                type === 'service' && 
                    filteredAndSortedList.map((item: any, index:number) => (
                        <div className="w-full flex items-center justify-start gap-2 bg-background px-1 py-2 rounded-lg  " onClick={() => setSelect(item)} key={item.name+index}>
                            <Stars
                            ...
                            >
                                <span className="text-lg font-bold w-full text-center text-tertiary ">{item.name}</span>
                            </button>
                        </div>
                    ))
            }

            {
                type === 'country' && 
                    filteredAndSortedList.map((item: any, index:number) => {
                        ...
                        return (
                            <div className="w-full flex items-center justify-start gap-2" key={item.country_name + index}>
                                <Stars
