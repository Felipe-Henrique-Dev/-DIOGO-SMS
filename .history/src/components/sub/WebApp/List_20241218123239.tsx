        ...
        if (type === 'service') {
            ...
            if(filterSelect) {
                ...
                uniqueServices = uniqueServices.filter((item: any) => {
                    ...
                    return filterSelect.services.map((service: any) => service.name).includes(item.name)
                })
            }
...

            {
                type === 'service' && 
                    filteredAndSortedList.map((item: any, index:number) => (
                        <div className="w-full flex items-center justify-start gap-2  px-2 py-2 rounded-lg shadow-md shadow-black border-t border-2 border-black hover:bg-secondary transition-all duration-300" key={item.name + index}>
                            <Stars
                                stars={favorites.includes(item.name)}
                                setFavorite={() => setFavoriteInStorage(item.name)}
                            />
                            <Image src={`${baseUrl}${item.logo}`} alt={item.name} width={30} height={30} onClick={() => setSelect(item)}  className="rounded-full cursor-pointer shadow-md shadow-black min-w-8 h-8 " />
                            <span className="text-lg font-bold w-full text-start text-black cursor-pointer " onClick={() => setSelect(item)} >{item.name}</span>
                        </div>
                    ))
            }

            {
                type === 'country' && 
                    filteredAndSortedList.map((item: any, index:number) => {
                        ...
                        return (
                            <div className="w-full flex items-center justify-start gap-2 bg-secondary px-2 py-2 rounded-lg " key={item.country_name + index}>
                                <Stars
