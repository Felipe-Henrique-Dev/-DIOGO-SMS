...
export default function WebApp() {
  ...
  return (
    <aside className="min-w-[25rem] min-h-[90svh] shadow-lg shadow-black bg-background_secondary text-pri sticky left-0 top-[4.5rem] rounded-lg flex flex-col p-2 gap-5  pt-14 z-[999]">
      ...
      <Container subTitle="1. Selecione o serviço" type={'servicos'} select={selectService} setSelect={setSelectService} />
      <Container subTitle="2. Selecione o pais" type={'paises'} select={selectCountry} setSelect={setSelectCountry} />
      <Container subTitle="3. Selecione a operadora" type={'operadora'} select={selectOperator} setSelect={setSelectOperator} />
    </aside>
  );
}
        ...
        <>
          ...
          <div className="h-96 shadow-inner shadow-black py-2 w-full my-2 overflow-x-auto scroll-smooth rounded-lg p-1 text-black font-ubuntu ">
            <Suspense fallback={<div>Carregando...</div>}>
              <List type={type} filter={filter} favorites={favorites} setFavorites={setFavorites} setSelect={setSelect} />
            </Suspense>
          </div>
        </>
      }
