...
export const WebApp = ( { country, service }: { country: any, service: any }) => {
  ...
  return (
    <aside className="min-w-[25rem] min-h-[90svh] shadow-lg shadow-black bg-background_secondary text-pri sticky left-0 top-[4.5rem] rounded-lg flex flex-col p-2 gap-5  pt-14 z-[999]">
      ...
      <Container subTitle="1. Selecione o serviço'" select={selectCountry} setSelect={setSelectCountry} type={'service'} country={country} service={service} filter={selectCountry} />
      <Container subTitle="2. Selecione o pais" select={selectService} setSelect={setSelectService} type={'country'} country={country} service={service} filter={selectService} />
